...
// GET OUR PDF FILE
pdfjsLib.getDocument("rivista.pdf").then((pdf) => {
...
document.getElementById("next").addEventListener("click", (e) => {
  if (defaultState.pdf == null || defaultState.currentPage > defaultState.pdf._pdfInfo.numPages) return;
  defaultState.currentPage += 1;
  document.getElementById("current_page").value = defaultState.currentPage;
  render();
});
...
document.getElementById("current_page").addEventListener("keypress", (e) => {
  ...
  if (code == 13) {
    // ON CLICK ENTER GO TO SITE TYPED IN TEXT-BOX
